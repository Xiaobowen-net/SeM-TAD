...
class TransformerVar(nn.Module):
    ...
    def __init__(self, win_size, enc_in, c_out, n_memory,
                 tau, contras_temperature, update_K, update_tau,
                 zero_probability, read_K,read_tau,
                 shrink_thres=0, \
        ...
        self.mem_module = MemoryModule(n_memory=n_memory, fea_dim=d_model,
                                       tau=tau, contras_temperature =contras_temperature, update_K=update_K, update_tau=update_tau, zero_probability=zero_probability, read_K=read_K,
                                       read_tau=read_tau, shrink_thres=shrink_thres, device=device, memory_init_embedding=memory_init_embedding, phase_type=phase_type, dataset_name=dataset_name)
    ...
    def forward(self, x):
        ...
        out, memory_item_embedding ,contrastloss ,gather_loss= outputs['output'], outputs['memory_init_embedding'],outputs['contrastloss'],outputs['gather_loss']
        ...
        if self.memory_initial:
            ...
            #########################################################################
            # similarities = self.cosine_similarity(queries[1, :, :].unsqueeze(1), queries[1, :, :].unsqueeze(0))
            # pairwise_distances = torch.cdist(queries[1, :, :], queries[1, :, :], p=2)
            # heat_map(similarities.detach().cpu().numpy(), self.phase_type, self.count)
            #########
            #计算查询和每个记忆项的权重
            # att_mermory_heat_map(attn[1, :, :].detach().cpu().numpy(), self.phase_type, self.count)
            # att_mermory_heat_map(attn[25, :, :].detach().cpu().numpy(), self.phase_type, self.count)
            # self.count += 1
            ...
            '''
            out :更新后的查询向量 N x L x 2C or N x 2C
            memory_init_embedding :更新后的记忆项
            queries :更新前的查询向量 N x L x C or N x C
            mem :记忆项

            attn :查询和每个记忆项的权重 N x L x M or N x M
            '''
            return {"out":out, "memory_item_embedding":memory_item_embedding, "queries":queries, "mem":mem,"contrastloss":contrastloss,"gather_loss":gather_loss}
